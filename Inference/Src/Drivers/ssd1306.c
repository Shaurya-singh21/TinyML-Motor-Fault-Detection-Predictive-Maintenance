...
int oled_cmd(uint8_t cmd)
{
	...
	OLED_I2C->DR = OLED_ADDR;

	t = OLED_CMD_TIMEOUT;

	while (!(OLED_I2C->SR1 & I2C_SR1_ADDR))
	{
		if (OLED_I2C->SR1 & I2C_SR1_AF)
		{
			OLED_I2C->SR1 &= ~I2C_SR1_AF;
			OLED_I2C->CR1 |= I2C_CR1_STOP;
			return -2;
		}
...
{
	uint8_t idx =
		(c >= 0x20 && c <= 0x7E) ? (uint8_t)(c - 0x20) : (uint8_t)('?' - 0x20);
...
void oled_flush(void)
{
	...
	while (OLED_I2C->CR1 & I2C_CR1_STOP)
		;

	OLED_I2C->CR1 |= I2C_CR1_START;
	uint32_t t = 50000;
	while (!(OLED_I2C->SR1 & I2C_SR1_SB) && --t)
		;
	if (!t)
		goto fail;
	...
	OLED_I2C->DR = OLED_ADDR;
	t = 50000;
	while (!(OLED_I2C->SR1 & I2C_SR1_ADDR) && --t)
		;
	if (!t)
	{
		OLED_I2C->CR1 |= I2C_CR1_STOP;
	...
	return; // lock is released later, by oled_dma_complete() — not here
