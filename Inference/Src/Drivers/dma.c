...
void dma_init(void)
{
	...
	// dma_1_stream4_channel3 for i2c3_tx for oled
	// (RM0390 DMA1 request mapping: I2C3_TX is Stream4/Channel3. Stream4 lives in
	//  the *high* status registers HISR/HIFCR, not LISR/LIFCR.)
	DMA1_Stream4->CR &= ~DMA_SxCR_EN;
	while (DMA1_Stream4->CR & DMA_SxCR_EN)
		;
	DMA1_Stream4->PAR = (uint32_t)&I2C3->DR;
	DMA1_Stream4->CR &= ~((7U << DMA_SxCR_CHSEL_Pos) | (3U << DMA_SxCR_DIR_Pos) | (1U << DMA_SxCR_CIRC_Pos));
	DMA1_Stream4->CR |= (3U << DMA_SxCR_CHSEL_Pos); // Channel 3
	DMA1_Stream4->CR |= (1U << DMA_SxCR_DIR_Pos);	// Memory to Peripheral

	DMA1_Stream4->CR |= DMA_SxCR_MINC | DMA_SxCR_TCIE; // Mem increment, Transfer Complete Interrupt
	DMA1_Stream4->CR &= ~(DMA_SxCR_MSIZE | DMA_SxCR_PSIZE);

	NVIC_SetPriority(DMA1_Stream4_IRQn, DMA1_STREAM4_CHANNEL3_PRIORITY);
	...
	DMA1_Stream6->CR &= ~((7U << DMA_SxCR_CHSEL_Pos) | (3U << DMA_SxCR_DIR_Pos) | (1U << DMA_SxCR_CIRC_Pos));
...
void DMA1_Stream4_IRQHandler(void)
{
	...
	if (DMA1->HISR & DMA_HISR_TCIF4)
	{
		...
		uint32_t t = I2C_FLAG_TIMEOUT;
		while (!(I2C3->SR1 & I2C_SR1_BTF) && --t)
			;
